...
from ..details.model import (
    Authentification,
    HotelOrderDetailQueryRequest,
    HotelOrderDetailQueryResponse,
    TicketRequest,
    TicketResponse,
)


class HotelOrderDetailQuery(BaseClient):
    """Query hotel settlement details through Ctrip's settlement API."""

    API_BASE_URL = "https://ct.ctrip.com"
    TICKET_PATH = "/SwitchAPI/Order/Ticket"
    SETTLEMENT_PATH = "/switchapi/SettlementHltOrder/SearchSettlementHltOrderDetail"
    ...
    def build_query_request(self, *, ticket: str | None = None, app_key: str | None = None, **kwargs) -> HotelOrderDetailQueryRequest:
        return HotelOrderDetailQueryRequest(Auth=self.build_auth(ticket, app_key) if ticket else None, **kwargs)
    ...
    async def query(self, request: HotelOrderDetailQueryRequest) -> HotelOrderDetailQueryResponse:
        ...
        async with httpx.AsyncClient(base_url=self.API_BASE_URL, limits=self.limits, timeout=self.timeout, transport=self.transport) as client:
            ...
            response.raise_for_status()
            return HotelOrderDetailQueryResponse.model_validate(response.json())
    ...
    async def query_by_batch_no(self, batch_no: str, **kwargs) -> HotelOrderDetailQueryResponse:
        return await self.query(self.build_query_request(BatchNo=batch_no, **kwargs))

    async def query_by_sub_batch_nos(self, sub_batch_nos: Sequence[str], **kwargs) -> HotelOrderDetailQueryResponse:
        return await self.query(self.build_query_request(SubBatchNoList=list(sub_batch_nos), **kwargs))

    async def query_by_record_ids(self, record_ids: Sequence[int | str] | int | str, **kwargs) -> HotelOrderDetailQueryResponse:
    ...
    async def query_by_order_id(self, order_id: int, **kwargs) -> HotelOrderDetailQueryResponse:
        return await self.query(self.build_query_request(OrderId=order_id, **kwargs))

    async def query_by_pre_approval_nos(self, approval_nos: Sequence[str], **kwargs) -> HotelOrderDetailQueryResponse:
