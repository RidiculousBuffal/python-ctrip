...
from ..details.model import (
    ...
    TicketRequest,
    TicketResponse,
    TrainOrderDetailsQueryRequest,
    TrainOrderDetailsQueryResponse,
)


class TrainOrderDetailsQuery(BaseClient):
    """Query train settlement details through Ctrip's settlement API."""

    API_BASE_URL = "https://ct.ctrip.com"
    TICKET_PATH = "/SwitchAPI/Order/Ticket"
    SETTLEMENT_PATH = "/switchapi/SettlementTrainOrder/SearchSettlementTrainOrderDetail"
    ...
    def build_query_request(self, *, ticket: str | None = None, app_key: str | None = None, **kwargs) -> TrainOrderDetailsQueryRequest:
        return TrainOrderDetailsQueryRequest(Auth=self.build_auth(ticket, app_key) if ticket else None, **kwargs)
    ...
    async def query(self, request: TrainOrderDetailsQueryRequest) -> TrainOrderDetailsQueryResponse:
        ...
        async with httpx.AsyncClient(base_url=self.API_BASE_URL, limits=self.limits, timeout=self.timeout, transport=self.transport) as client:
            ...
            response.raise_for_status()
            return TrainOrderDetailsQueryResponse.model_validate(response.json())
    ...
    async def query_by_batch_no(self, batch_no: str, *, settlement_type: str | None = None, **kwargs) -> TrainOrderDetailsQueryResponse:
        return await self.query(self.build_query_request(BatchNo=batch_no, SettlementType=settlement_type, **kwargs))

    async def query_by_sub_batch_nos(self, sub_batch_nos: Sequence[str], **kwargs) -> TrainOrderDetailsQueryResponse:
        return await self.query(self.build_query_request(SubBatchNoList=list(sub_batch_nos), **kwargs))

    async def query_by_record_ids(self, record_ids: Sequence[int | str] | int | str, **kwargs) -> TrainOrderDetailsQueryResponse:
    ...
    async def query_by_order_id(self, order_id: int, **kwargs) -> TrainOrderDetailsQueryResponse:
